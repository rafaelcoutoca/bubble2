...
interface HeroProps {
  onSearchClick: () => void;
  /** chamado ao clicar em “Cadastro” */
  onSignupClick?: () => void; // opcional pra não quebrar usos antigos
}
...
const Hero: React.FC<HeroProps> = ({ onSearchClick, onSignupClick }) => {
  return (
    <div className="relative pt-16 pb-8 md:pb-12 bg-gradient-to-br from-primary-900 via-primary-800 to-dark-900 overflow-hidden">
      ...
      <div className="absolute inset-0">
        <div className="absolute top-20 left-10 w-32 h-32 bg-accent-500 rounded-full opacity-10 animate-bounce-slow" />
        <div className="absolute top-40 right-20 w-24 h-24 bg-accent-400 rounded-full opacity-20 animate-pulse-slow" />
        <div className="absolute bottom-20 left-1/3 w-16 h-16 bg-accent-300 rounded-full opacity-15 animate-bounce" />
      </div>
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-20">
        <div className="text-center md:text-left">
          ...
          <p className="text-xl md:text-2xl text-gray-200 max-w-2xl mx-auto md:mx-0 mb-8 leading-relaxed">
            Participe de competições perto de você, acompanhe seus resultados e
            viva a evolução no esporte
          </p>
          ...
          <div className="flex flex-col sm:flex-row justify-center md:justify-start space-y-4 sm:space-y-0 sm:space-x-6">
            ...
            <button
              ...
              className="bg-transparent border-2 border-accent-500 text-accent-500 hover:bg-accent-500 hover:text-dark-900 px-8 py-4 rounded-xl font-bold text-lg transition-all duration-300 shadow-lg hover:shadow-xl"
            >
              Cadastro
            </button>
          </div>
