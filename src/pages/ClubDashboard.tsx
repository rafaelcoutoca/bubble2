...
const ClubDashboard: React.FC = () => {
  ...
  useEffect(() => {
    // Simulação para testes de layout — pode remover depois
    ...
    if (savedClubData) {
      ...
      const isIncomplete =
        !parsedData.fantasyName || !parsedData.description || !parsedData.city;
  ...
  const totalParticipants = clubTournaments.reduce(
    (sum: number, tournament: any) => {
      return sum + (tournament.participantsCount || 0);
    },
    0
  );

  const activeTournaments = clubTournaments.filter(
  ...
  const recentActivities = [
    {
      id: 1,
      action: "Novo torneio criado",
      tournament: "Copa Verão 2024",
      time: "2 horas atrás",
      type: "create",
    },
    {
      id: 2,
      action: "Grupo A gerado",
      tournament: "Open Padel",
      time: "5 horas atrás",
      type: "group",
    },
    {
      id: 3,
      action: "Resultado adicionado",
      tournament: "Championship Elite",
      time: "1 dia atrás",
      type: "result",
    },
    {
      id: 4,
      action: "5 novas inscrições",
      tournament: "Torneio Iniciantes",
      time: "2 dias atrás",
      type: "registration",
    },
  ];
          ...
          <div className="mb-6 bg-accent-50 border border-accent-200 rounded-lg p-4">
            <div className="flex items-start">
              ...
              <div className="flex-1">
                <h3 className="text-accent-800 font-medium mb-1">
                  Complete seu perfil
                </h3>
                <p className="text-accent-700 text-sm mb-3">
                  Para aproveitar todas as funcionalidades, complete seu
                  cadastro com as informações do seu clube.
                </p>
                <Link
                  to="/settings"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-8">
          {kpis.map((kpi, index) => (
            ...
            >
              ...
              <div>
                <h3 className="text-dark-600 text-sm font-medium mb-1">
                  {kpi.title}
                </h3>
                <p className="text-3xl font-bold text-dark-900 mb-2">
                  {kpi.value}
                </p>
                <p
                  className={`text-xs ${
              ...
              {nextTournaments.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-dark-500 mb-4">
                    Crie seu próximo torneio para começar.
                  </p>
                  <Link
                    to="/create-tournament"
